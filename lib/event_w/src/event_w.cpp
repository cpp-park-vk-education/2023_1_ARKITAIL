...
void EventW::makeEventWidget(Wt::WTable *table, Wt::WDate begin_of_week) {
    ...
    if (begin_.date().daysTo(end_.date()) || end_.time().hour() - begin_.time().hour() == 24) {
        ...
        auto end_event = end_.date() < begin_of_week.addDays(6) ? end_.date() : begin_of_week.addDays(6);

        bool event_time = false;
        std::string base_style = "w-100 ";
        for (auto day = begin_of_week; day < begin_of_week.addDays(7); day = day.addDays(1)) {
            if (day == end_event) {
                auto style = base_style + (day == begin_.date() ? "rounded-start" : "border-start-0") +
                ...
                auto style =
                    base_style + (day == end_event ? "rounded-end" : "border-end-0") + (std::string) " rounded-start";
...
void EventW::makeEventLargePartWidget(std::string title, std::string style_class, Wt::WTableCell *event_cell) {
    ...
    eventWidget->setStyleClass("p-0 border-0 text-truncate btn btn-sm btn-light rounded-0 " + style_class);
...
void EventW::makeEventSmallPartWidget(Wt::WTableCell *event_cell) {
    ...
    eventWidget->decorationStyle().setBackgroundColor(color_);
    eventWidget->setStyleClass("p-1 px-2 m-1 border-0 text-truncate btn btn-sm btn-light rounded-0");
    addDialog(eventWidget);
