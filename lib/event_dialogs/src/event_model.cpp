...
std::shared_ptr<Event> EventModel::GetData() const {
  ...
  event->summary = Wt::asString(value(kSummary));
  event->description = Wt::asString(value(kDescription));
  event->calendar = Wt::asString(value(kCalendars));

  event->start = Wt::WDateTime(
      Wt::cpp17::any_cast<Wt::WDate>(value(kStartDate)),
      Wt::cpp17::any_cast<Wt::WTime>(value(kStartTime)));
  event->end = Wt::WDateTime(
      Wt::cpp17::any_cast<Wt::WDate>(value(kEndDate)),
      Wt::cpp17::any_cast<Wt::WTime>(value(kEndTime)));

  event->is_recurrent = Wt::cpp17::any_cast<bool>(value(kIsRecurrent));
  event->frequency = Wt::asString(value(kFrequency));
  event->interval = Wt::cpp17::any_cast<std::size_t>(value(kInterval));
  event->until = Wt::cpp17::any_cast<Wt::WDate>(value(kEndDate));
