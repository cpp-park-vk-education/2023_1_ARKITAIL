...
// const char* conninfo = "host=localhost port=5432 dbname=for_project_ARKITAIL user=antiho password=qwerty";
...
//       std::cout << "Query execution failed second: " << PQerrorMessage(conn) << std::endl;
...
//     std::string s = "INSERT INTO " + p.table + " ( " + p.id +  p.columns + " ) VALUES ((SELECT COALESCE(MAX( " + p.id + " ), 0) FROM users) + 1, " + p.values;
//     const char* sql = s.c_str();
//     // Выполняем запрос по выводу значения всей таблицы
//     res = PQexec(conn, sql);
...
//     {
//         // Выводим сообщение об ошибке
//         std::cout << "Query execution failed first: " << PQerrorMessage(conn) << std::endl;
...
//   insert i = {"users", "user_id", ", name, email, password, avatar", "'tester', 'test.l.2004@yandex.ru', 'qwerty', NULL);"};
//   InsertFunction(i);
...
time_t set_time_start(){
    std::tm timeinfo = {};
    timeinfo.tm_year = 2023 - 1900; // год, отнимаем 1900 для приведения к формату tm
...
time_t set_time_end(){
    std::tm timeinfo = {};
    timeinfo.tm_year = 2024 - 1900; // год, отнимаем 1900 для приведения к формату tm
    timeinfo.tm_mon = 4; // май (нумерация месяцев начинается с 0)
    timeinfo.tm_mday = 2; // день месяца
    timeinfo.tm_hour = 12; // часы
    timeinfo.tm_min = 30; // минуты
...
int main(){
    ...
    // event->description = "random description";
    // calendarPtr = session.find<calendars>().where("name = ?").bind("calendar1");
    // event->calendar = calendarPtr;
    // dbo::ptr<events> eventPtr = session.add(std::move(event));
    ...
    //     std::cout << user->login << ", " << user->email << ", " << user->password << std::endl;
    // }
